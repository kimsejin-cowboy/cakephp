<?php /* @var $this View */ ?>

<!-- Modal -->
<div id="UserModal" class="modal-wrapper">
	<div class="modal">
		ユーザー新規追加・更新
		<?php echo $this->Form->create('UserPost', array(
						'type'=>'post',
						'url'=>array('controller'=>'users', 'action'=>'search'))); ?>
		<?php $this->Form->inputDefaults(array('div'=>false, 'label'=>false, 'required'=>false)); ?>
	    <table>
	        <tr>
	            <td><div class="text-label">ログインID</div></td>
	            <td>
	            <?php echo $this->Form->input('user_name', array('type'=>'text')); ?>
	            </td>
	            <td>
	            <?php echo $this->Form->input('admin', array('type'=>'checkbox', 'value'=>'1')); ?> 管理者ユーザー
	            </td>
	        </tr>
	        <tr>
	            <td><div class="text-label">パスワード</div></td>
	            <td>
	            <?php echo $this->Form->input('user_password', array('type'=>'password')); ?>
	            </td>
	            <td></td>
	        </tr>
	    </table>
	    <?php echo $this->Form->hidden('id', array('value'=>0)); ?>
		<?php echo $this->Form->button('登録',
				array('type'=>'button', 'id'=>'UserSaveButton', 'class'=>'link-label', 'name'=>'submit')); ?>
		<?php echo $this->Form->button('閉じる',
				array('type'=>'button', 'class'=>'closeModal link-label', 'type'=>'button')); ?>
		<?php echo $this->Form->end(); ?>
	</div>
</div>

<!-- Title -->
<div id="PageNavigator">
<?php echo $this->Html->link('INDEX', array('controller'=>'tops', 'action'=>'index')); ?>
 > <?php echo $this->Html->link('Topic', array('controller'=>'topics', 'action'=>'search')); ?>
 > <?php echo $this->Html->link('Manager', array('controller'=>'tops', 'action'=>'manager')); ?>
 > User
 </div>
<h3>ユーザー一覧・検索</h3>

<!-- Form -->
<div id="UserSearchForm">
	<?php echo $this->Form->create('UserGet', array(
					'type'=>'get',
					'url'=>array('controller'=>'users', 'action'=>'search'))); ?>
	<?php $this->Form->inputDefaults(array('div'=>false, 'label'=>false, 'required'=>true)); ?>
	<table >
        <tr>
            <td width="180"><div class="text-label">ユーザー名</div></td>
            <td>
            <?php echo $this->Form->input('user_name',
            		array('type'=>'text', 'class'=>'small-form')); ?>
            </td>
        </tr>
    </table>
	<?php echo $this->Form->button('検索',
			array('type'=>'submit', 'class'=>'link-label','name'=>'search')); ?>
	<?php echo $this->Form->button('新規ユーザー追加',
			array('type'=>'button', 'class'=>'openFilledModal link-label')); ?>
	<?php echo $this->Form->end(); ?>
</div>

<!-- Paginator -->
<div id="PaginationLinkHead" class="pagination-link">
    <?php echo $this->Paginator->first('<<'); ?>
    <?php echo $this->Paginator->prev('<'); ?>
    <?php echo $this->Paginator->numbers(); ?>
    <?php echo $this->Paginator->next('>'); ?>
    <?php echo $this->Paginator->last('>>'); ?>
</div>
<div id="UserSearchResult">
    <table class="manage">
        <thead>
            <tr>
                <th width="200">詳細</th>
                <th width="400">ログインID</th>
                <th width="200">管理者ユーザー</th>
                <th width="200">削除</th>
            </tr>
        </thead>
        <tbody>
        <?php foreach ($users as $u) { ?>
            <tr>
                <td><?php echo $this->Html->div('link-label openFilledModal', '詳細', array('name'=>$u['User']['id'])); ?></td>
                <td><?php echo $u['User']['user_name']; ?></td>
                <td><?php echo $adminList[ $u['User']['admin'] ]; ?></td>
                <td>
                <?php echo $this->Form->create('UserDelete', array(
					    'type'=>'post',
					    'url'=>array('controller'=>'users', 'action'=>'deleteUser'),
					    'onsubmit'=>'return confirmBox();'
					    )); ?>
				<?php echo $this->Form->hidden('id', array('value'=>$u['User']['id'])); ?>
				<?php echo $this->Form->button('削除', array('type'=>'submit', 'class'=>'link-label', 'name'=>'delete')); ?>
				<?php echo $this->Form->end(); ?>
                </td>
            </tr>
        <?php } ?>
        </tbody>
    </table>
</div>
<div id="PaginationLinkFoot" class="pagination-link">
    <?php echo $this->Paginator->first('<<'); ?>
    <?php echo $this->Paginator->prev('<'); ?>
    <?php echo $this->Paginator->numbers(); ?>
    <?php echo $this->Paginator->next('>'); ?>
    <?php echo $this->Paginator->last('>>'); ?>
</div>

<script type="text/javascript">
<!--
$(document).ready(function () {
	MODAL.init('User');
});
-->
</script>


